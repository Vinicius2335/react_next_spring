...
export default function TablePermissoes() {
  ...
  function carregaDados() {
    PermissaoService.getAllPermissoes()
      .then(data => {
        setIsLoading(false)
        setData(data)
      })
  ...
  function onSalvarEdit() {
    let permissaoToEdit = permissao
    permissaoToEdit.nome = nomePermissao

    if (permissaoToEdit.id !== null) {
      PermissaoService.alterar(permissaoToEdit, permissaoToEdit.id)
        .then(() => {
        ...
        .catch(() => {
          toast.error("Erro ao tentar editar permissão, tente novamente mais tarde!")
        })
    }
  }
  ...
  function onConfirmarDelete() {
    let permissaoToDelete = permissao
    PermissaoService.delete(permissaoToDelete.id)
      .then(() => {
      ...
      .catch(() => {
        toast.error("Erro ao tentar deletar permissão, tente novamente mais tarde!")
      })
  }

  function onSalvarAdd() {
    let permissaoToAdd: Partial<DataTypePermissao> = {
      nome: nomePermissao
    }

    PermissaoService.inserir(permissaoToAdd)
      .then(() => {
      ...
      .catch(() => {
        toast.error("Erro ao tentar salvar permissão, tente novamente mais tarde!")
      })
  }
      ...
      case "actions":
        return (
          <div className="relative flex justify-start items-center gap-4">
            <Tooltip content="Edit permissão">
              <span
                onClick={() => onEdit(item)}
            ...
            <Tooltip color="danger" content="Delete permissão">
              <span
  ...
  return (
    <>
      <Table
        aria-label="DataGrid de Permissões"
        isHeaderSticky
      ...
      >
        ...
        <TableBody
          isLoading={isLoading}
          emptyContent={isEmptyContent ? "No Permissões found" : ""}
          items={sortedItems}
          loadingContent={
